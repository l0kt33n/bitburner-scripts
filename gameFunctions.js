...
//  whichever is less
export function growAnalyze(server, player, growth, BitNodeMultipliersServerGrowthRate, difficulty = -1, cores = 1, capGrowMult = false) {
    if (difficulty < 0) {
        difficulty = server.minDifficulty;
    } // Assume min security
    else if (difficulty == 0) {
        difficulty = server.hackDifficulty;
    } // Use current security
...
export function growAnalyzeHackAmount(server, player, hackAmt, BitNodeMultipliersServerGrowthRate, difficulty = -1, cores = 1, capGrowMult = false) {
...
export function getHackTime(server, player, difficulty = -1) {
    if (difficulty <= 0) {
        difficulty = server.minDifficulty;
    }
...
export function getGrowTime(server, player, difficulty = -1) {
    const growTimeMultiplier = 3.2; // Relative to hacking time. 16/5 = 3.2
    return growTimeMultiplier * getHackTime(server, player, difficulty);
}
export function getWeakenTime(server, player, difficulty = -1) {
    const weakenTimeMultiplier = 4; // Relative to hacking time
    return weakenTimeMultiplier * getHackTime(server, player, difficulty);
}
export function hackAnalyzeChance(server, player, difficulty = -1) {
    if (difficulty < 0) {
        difficulty = server.minDifficulty;
    } // Assume min security
    else if (difficulty == 0) {
...
export function hackAnalyze(server, player, BitNodeMultipliersScriptHackMoney, difficulty = -1) {
    if (difficulty < 0) {
        difficulty = server.minDifficulty;
    } // Assume min security
    else if (difficulty == 0) {
