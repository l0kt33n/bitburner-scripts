// @ts-nocheck
export function getHTime(server, player = Math.asd.player, difficulty = -1) {
    if (difficulty <= 0) {
...
export function hAnalyze(server, player = Math.asd.player, difficulty = -1) {
    ...
    const skillMult = (player.hacking - (server.requiredHackingSkill - 1))
        / player.hacking;
    const percentMoneyHacked = (difficultyMult * skillMult * player.hacking_money_mult * (globalThis["pigalot-data"]?.gameInfoData?.BitNodeMultipliers?.ScriptHackMoney ?? 1.0)) / balanceFactor;
...
export function gAnalyze(server, player = Math.asd.player, growth, difficulty = -1, cores = 1, capGrowMult = false) {
    ...
    const threadMultiplier = serverGrowthPercentage * player.hacking_grow_mult * coreMultiplier * (globalThis["pigalot-data"]?.gameInfoData?.BitNodeMultipliers?.ServerGrowthRate ?? 1); //total of all grow thread multipliers
