    ...
    async run() {
        while (true) {
            ...
            for (const server of this.spiderData.routedServers) {
                if (!StaticConfig.UseHacknetServers && server.name.startsWith("hacknet-node-"))
                    continue;
            ...
            if (!this.resourceManagerData.initialised)
                this.resourceManagerData.initialised = true;
