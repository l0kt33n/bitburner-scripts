...
class TargeterDeamon extends Deamon {
            ...
            for (const target of servers) {
                ...
                do {
                    ...
                    if (detail) {
                        ...
                        if (ret.amountHacked < 0.01) {
                            hN += hN < 10 ? 1 : hN < 100 ? 10 : Math.ceil(0.0001 / ret.hackAmount);
                        } //step by 0.01%
                        else {
                            hN += Math.ceil(0.02 / ret.hackAmount);
                        } //step by 0.2%
                    }
    ...
    async run2() {
        ...
        do {
            ...
            for (const tardat of dats) {
                ...
                let hN = 1;
                let ret = { tar: tar, value: 0, score: 0, cost: 0, hN: 1, cL: StaticConfig.minimumCycleLength };
                const maxCost = StaticConfig.RamUse * this.resourceManagerData.totalRam;
                let bi = bests.findIndex(a => a.tar == tar);
                if (bi == -1) {
                    bi = bests.length;
                    bests.push(ret);
                } // add the new target to the profiles list
                else if ((maxCost - bests[bi].cost < 0) //overRam, remove profile
                ...
                //				|| (bests[bi].cL -0.9*asd.realcL < 0) //to far below realcL, remove profile
                    ...
                    bests[bi] = ret;
                } //check that profile is still valid
                do {
                    await this.ns.sleep(0);
                    //check for a better target
                    ...
                    if (count % 10 == 9) {
                        ...
                        if (ret.amt < 0.01) {
                            hN += hN < 10 ? 1 : hN < 100 ? 10 : Math.ceil(0.0001 / hA);
                        } //step by 0.01%
                        else {
                            hN += Math.ceil(0.02 / hA);
                        } //step by 0.2%
                    }
    ...
    estBestScore(tar, hN, tardat, cL = StaticConfig.minimumCycleLength, cores = 1) {
     ...
     */
    estimateBestScore(target, hackThreads, player, bufferTime = this.minBufferTime, cores = 1) {
