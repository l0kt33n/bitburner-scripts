    ...
    async run() {
        if (!this.ns.bladeburner.joinBladeburnerDivision()) {
            ...
            if (player.bitNodeN === 8)
                return; // cant do this on 8
            while (player.strength < 100 || player.defense < 100 || player.dexterity < 100 || player.agility < 100) {
                await this.ns.sleep(10 * 1000);
            }
            if (!this.ns.bladeburner.joinBladeburnerDivision())
                return; // how have we got here?
            else if (this.ns.bladeburner.getRank() > 25)
                this.ns.bladeburner.joinBladeburnerFaction();
        }
        ...
        const blackOps = this.ns.bladeburner.getBlackOpNames().map(b => {
            ...
            if (maxRequiredRank < requiredRank)
                maxRequiredRank = requiredRank;
            ...
            for (const bop of blackOps) {
                ...
                if (average !== 1) {
                    if (isFirst) {
                        ...
                        if (count == 2)
                            this.ns.print(`bo: ${bop.name} took too long to analyse getting back on with our lives`);
                    }
                    continue;
                }
